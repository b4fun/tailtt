...
type Notifier interface {
	...
	Notify(ctx context.Context, filename, line string) error
}
...
func (n notifier) Notify(ctx context.Context, filename, line string) error {
	for _, sub := range n {
		if err := sub.Notify(ctx, filename, line); err != nil {
			return err
		}
	}
